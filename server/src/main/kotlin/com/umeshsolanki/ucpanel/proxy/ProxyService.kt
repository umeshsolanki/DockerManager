...
package com.umeshsolanki.ucpanel.proxy
...
import com.umeshsolanki.ucpanel.utils.ExecuteResult
...
import com.umeshsolanki.ucpanel.kafka.IKafkaService
...
class ProxyServiceImpl(
    ...
    override fun updateSecuritySettings(
        ...
        // Validate Regexes (skip for EXACT match rules — they are literal strings)
        val allRulesToCheck = (rules + (recommendedRules ?: emptyList()))
        for (rule in allRulesToCheck) {
            if (rule.matchMode == ProxyJailMatchMode.EXACT) continue
            if (rule.type == ProxyJailRuleType.PATH || rule.type == ProxyJailRuleType.USER_AGENT
                || rule.type == ProxyJailRuleType.HOST_HEADER || rule.type == ProxyJailRuleType.COMPOSITE
                || rule.type == ProxyJailRuleType.STATUS_CODE) {
                try {
                    Regex(rule.pattern)
                } catch (e: Exception) {
    ...
    private fun getDefaultNginxConfig(): String {
        ...
        val securityMapsContent = run {
            ...
            val pathViolations = settings.proxyJailRules
                ...
                .joinToString("\n    ") { rule ->
                    val desc = "\"${rule.description?.replace("\"", "\\\"") ?: "path_violation"}\""
                    if (rule.matchMode == ProxyJailMatchMode.EXACT) {
                        "\"${rule.pattern.replace("\"", "\\\"")}\" $desc;"
                    } else {
            ...
            val uaViolations = settings.proxyJailRules
                ...
                .joinToString("\n    ") { rule ->
                    val desc = "\"${rule.description?.replace("\"", "\\\"") ?: "ua_violation"}\""
                    if (rule.matchMode == ProxyJailMatchMode.EXACT) {
                        "\"${rule.pattern.replace("\"", "\\\"")}\" $desc;"
                    } else {
                        val pattern = rule.pattern.replace("\"", "\\\"")
