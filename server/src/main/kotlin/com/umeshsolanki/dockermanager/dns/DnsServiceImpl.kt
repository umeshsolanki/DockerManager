...
class DnsServiceImpl : IDnsService {
    ...
    private fun writeForwardersToConfig(config: DnsForwarderConfig) {
        ...
        // Ensure options block is updated or exists
        writeOptionsConfig()
    }

    private fun writeOptionsConfig() {
        val confDir = File(getNamedConfDir())
        val optionsFile = File(confDir, "named.conf.options")
        val forwardersPath = if (isDockerMode) "/etc/bind/named.conf.forwarders" else File(confDir, "named.conf.forwarders").absolutePath
        ...
        val content = buildString {
            ...
            appendLine("    ")
            appendLine("    // Response Rate Limiting (RRL) - Prevents DNS amplification attacks")
    ...
    private fun seedDefaultConfig(configDir: File) {
        ...
        val optionsPath = if (isDockerMode) "/etc/bind/named.conf.options" else File(configDir, "named.conf.options").absolutePath
        val localPath = if (isDockerMode) "/etc/bind/named.conf.local" else File(configDir, "named.conf.local").absolutePath
        val loggingPath = if (isDockerMode) "/etc/bind/named.conf.logging" else File(configDir, "named.conf.logging").absolutePath
