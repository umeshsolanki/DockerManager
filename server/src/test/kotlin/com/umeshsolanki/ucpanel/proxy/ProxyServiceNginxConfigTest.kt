...
class ProxyServiceNginxConfigTest {
    ...
    fun `ensureNginxMainConfig updates config when variables are missing`() {
        ...
        val jailManager = mockk<com.umeshsolanki.ucpanel.jail.IJailManagerService>(relaxed = true)
        val sslService = mockk<ISSLService>(relaxed = true)
        val kafkaService = mockk<com.umeshsolanki.ucpanel.kafka.IKafkaService>(relaxed = true)
