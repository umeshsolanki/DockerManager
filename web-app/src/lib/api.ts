...
export const DockerClient = {
    ...
    getHistoricalStats: (date: string) => req<ProxyStats | null>(`/proxy/stats/history/${date}`, {}, null),
    getStatsForDateRange: (startDate: string, endDate: string) => req<ProxyStats[]>(`/proxy/stats/history/range?start=${encodeURIComponent(startDate)}&end=${encodeURIComponent(endDate)}`, {}, []),
