...
import com.umeshsolanki.ucpanel.api.ComposeApiService
...
fun ComposeScreen() {
    ...
    LaunchedEffect(Unit) {
        isLoading = true
        composeFiles = ComposeApiService.listComposeFiles()
        isLoading = false
    }
    ...
    ComposeContent(
        ...
        onUp = { path ->
            scope.launch {
                isLoading = true
                ComposeApiService.composeUp(path)
                refresh()
            }
        },
        onDown = { path ->
            scope.launch {
                isLoading = true
                ComposeApiService.composeDown(path)
                refresh()
            }
        }
    )
